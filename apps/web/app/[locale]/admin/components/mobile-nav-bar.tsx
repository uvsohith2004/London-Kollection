...
export function MobileNavBar({ items, selectedIndex, onSelectIndex }: MobileNavBarProps) {
  ...
  const snap = (index: number, animate = true) => {
    ...
    if (animate) {
      gsap.to(trackRef.current, {
        ...
        onComplete: () => {
          if (selectedIndex !== index) onSelectIndex(index);
        }
      });
    } else {
  ...
  useEffect(() => {
    if (currentIndex.current !== selectedIndex && trackRef.current) {
      currentIndex.current = selectedIndex;
      gsap.to(trackRef.current, {
        x: getTargetX(selectedIndex),
        duration: 0.6,
        ease: "elastic.out(1, 0.75)",
      });
    }
  ...
  useEffect(() => {
    ...
    const down = (e: PointerEvent) => {
      ...
      // Detect which item was touched for click handling
    ...
    const up = () => {
      ...
      // Handle raw taps directly to bypass pointer capture swallowing clicks
  ...
  return (
    <div className="pb-safe relative w-full shrink-0 z-50 h-24 bg-background/80 backdrop-blur-lg border-t border-border/40 overflow-hidden select-none touch-none" ref={viewportRef}>
      <div className="absolute left-1/2 top-0 h-full w-0 flex items-center">
        <div ref={trackRef} className="flex h-full items-center will-change-transform">
          {items.map((item, idx) => (
            ...
            >
              {/* Outer Ring */}
              <div className="nav-outer-ring absolute inset-1 rounded-full  border-foreground/20 opacity-0 transition-none" />
              
              {/* Inner Circle */}
              <div className="nav-inner-circle absolute inset-3 flex items-center justify-center rounded-full bg-foreground/5 transition-none">
                <item.icon className="nav-icon h-5 w-5 text-muted-foreground transition-none" />
              </div>
            </div>
          ))}
        </div>
