...
import { useAdminCategoriesQuery } from "../../products/queries"
import { useDeleteCategoryMutation } from "../../products/mutations"
import { Button } from "@workspace/ui/components/button"
import { Input } from "@workspace/ui/components/input"
import { Badge } from "@workspace/ui/components/badge"
import { CategoryForm } from "./category-editor-dialog"

export function CategoriesTab() {
  const [search, setSearch] = React.useState("")
  const [selectedCategory, setSelectedCategory] = React.useState<any>(null)

  const { data: response, isLoading } = useAdminCategoriesQuery()
  ...
  const items = categories.filter((c: any) => c.name?.toLowerCase().includes(search.toLowerCase()))
      ...
      {(isCreating || editingCategory) && (
        <div className="mb-8 animate-in fade-in slide-in-from-top-4 duration-300">
          <CategoryForm 
            ...
            onSuccess={() => { setIsCreating(false); setEditingCategory(null); }} 
      ...
      <div className="rounded-2xl border border-border/40 bg-card p-1 shadow-sm">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center p-16 text-center">
            ...
            <h4 className="text-lg font-medium">No Categories Found</h4>
            <p className="mt-2 max-w-sm text-sm text-muted-foreground">
              Categories help organize your catalog into a hierarchy.
            </p>
