...
import { useAdminOccasionsQuery } from "../../products/queries"
import { useDeleteOccasionMutation } from "../../products/mutations"
import { Button } from "@workspace/ui/components/button"
import { Input } from "@workspace/ui/components/input"
import { Badge } from "@workspace/ui/components/badge"
import { OccasionForm } from "./occasion-editor-dialog"

export function OccasionsTab() {
  const [search, setSearch] = React.useState("")
  const [isCreating, setIsCreating] = React.useState(false)
  const [editingOccasion, setEditingOccasion] = React.useState<any>(null)

  const { data: response, isLoading } = useAdminOccasionsQuery()
  const { mutate: deleteOccasion } = useDeleteOccasionMutation()

  const occasions = response?.items || []
  const items = occasions.filter((c: any) => 
    c.name?.toLowerCase().includes(search.toLowerCase()) || 
    c.title?.toLowerCase().includes(search.toLowerCase())
  )

  const handleEdit = (occasion: any) => {
    // API returns title, but we map it to name for consistency in form
    setEditingOccasion({ ...occasion, name: occasion.name || occasion.title })
    setIsCreating(false)
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <h3 className="text-xl font-medium tracking-tight">Occasions</h3>
          <p className="text-sm text-muted-foreground">
            Group products by events like Anniversaries, Birthdays, etc.
          </p>
        </div>
      ...
      {(isCreating || editingOccasion) && (
        <div className="mb-8 animate-in fade-in slide-in-from-top-4 duration-300">
          <OccasionForm 
            initialData={editingOccasion} 
            onSuccess={() => { setIsCreating(false); setEditingOccasion(null); }} 
            onCancel={() => { setIsCreating(false); setEditingOccasion(null); }}
          />
        </div>
      )}
      ...
      <div className="rounded-2xl border border-border/40 bg-card p-1 shadow-sm">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center p-16 text-center">
            ...
            <h4 className="text-lg font-medium">No Occasions Found</h4>
            <p className="mt-2 max-w-sm text-sm text-muted-foreground">
              Occasions help group products for special events.
            </p>
