...
export default function FeaturedPiecesPage() {
  ...
  const { mutate: setFeaturedPieces, isPending: isSaving } =
    useSetFeaturedPiecesMutation()
    ...
    if (over && active.id !== over.id) {
      setSortedItems((items) => {
        const oldIndex = items.findIndex(
          (i) => (i.id || i.productId) === active.id
        )
        const newIndex = items.findIndex(
          (i) => (i.id || i.productId) === over.id
        )
  ...
  const handleDeleteLocal = (idOrProductId: string) => {
    setSortedItems((items) => {
      const newItems = items.filter(
        (i) => (i.id || i.productId) !== idOrProductId
      )
  ...
  const handleAddProduct = (product: any) => {
    ...
    const newItems = [
      ...sortedItems,
      { productId: product.id, product, isActive: true },
    ]
                ...
                >
                  {sortedItems.map((item) => (
                    <SortableItem
                      ...
                      item={item}
                      onDelete={() =>
                        handleDeleteLocal(item.id || item.productId)
                      }
...
function AddFeaturedPieceForm({
  ...
  return (
    <div className="space-y-6">
      ...
      <StickySaveBar 
        onCancel={onCancel}
        onSave={() => onAdd(selectedProduct)}
        disabled={!selectedProduct}
        saveActionLabel="ADD PRODUCT"
      />
    </div>
  )
}
