        ...
        {isLoading ? (
          ...
          <div className="grid gap-8 md:grid-cols-2">
            ...
            <div className="space-y-2.5">
              <Label className="text-xs font-medium text-muted-foreground">End Time (Optional)</Label>
              <DateTimePicker
                value={endTime}
                onChange={handleSaveEndTime}
              />
              <p className="text-xs text-muted-foreground">If set, a countdown timer will appear.</p>
            </div>
          </div>
        )}
      </div>
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="flex justify-end gap-3 border-t border-border/40 pt-6">
        ...
        <Button type="submit" disabled={isPending || !watchProductId} className="font-bold tracking-widest uppercase">
          {isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
          {isEditing ? "Save Changes" : "Add to Flash Sale"}
        </Button>
      </div>
    </form>
  )
