...
export function ReturnFormBuilder() {
  ...
  const { data: forms = [], isLoading } = useQuery({
    queryKey: ["adminReturnForms"],
    queryFn: async () => {
      const res: any = await get("/admin/return-forms");
      return res?.forms || [];
    }
  });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: any) => {
      return post("/admin/return-forms", data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["adminReturnForms"] });
      toast.success("Return form created");
      resetForm();
    },
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: { id: string; payload: any }) => {
      return put(`/admin/return-forms/${data.id}`, data.payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["adminReturnForms"] });
      toast.success("Return form updated");
      resetForm();
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      return del(`/admin/return-forms/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["adminReturnForms"] });
      toast.success("Return form deleted");
  ...
  return (
        ...
        <Card className="p-6 space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Defect Returns" />
            </div>
            <div className="space-y-2">
          ...
          <div className="space-y-4 pt-4">
            ...
            {schema.length === 0 ? (
              <div className="p-8 border border-dashed rounded-lg text-center text-sm text-muted-foreground">
                No fields added yet. The user will only be asked basic return information.
              </div>
            ) : (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                          ...
                          <Input 
                            ...
                            placeholder="e.g. Please upload photos of the defect" 
                          />
                        </div>
                        <div className="space-y-2">
    ...
    <StickySaveBar 
        ...
        infoPanel={
          <div className="flex items-center gap-2">
            <span className="font-medium text-foreground">{editingFormId ? "Edit Return Form" : "New Return Form"}</span>
