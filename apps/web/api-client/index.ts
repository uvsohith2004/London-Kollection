...
export const fetchPersonalizedRecommendationsPaginated =
  async ({ pageParam = 0 }: { pageParam?: number }): Promise<{ items: Product[], nextOffset: number | null }> => {
    return await get(`/history/recommended?limit=20&offset=${pageParam}`)
  }
...
export const fetchOccasionBySlug = async (slug: string): Promise<Occasion> => {
  return await get(`/collections/occasions/slug/${slug}`)
}
...
export const fetchCollectionBySlug = async (slug: string): Promise<Collection> => {
  return await get(`/collections/slug/${slug}`)
}
...
export const fetchCategoryBySlug = async (slug: string) => {
  return await get(`/categories/slug/${slug}`)
}
...
export const fetchBrandBySlug = async (slug: string) => {
  return await get(`/admin/brands/slug/${slug}`)
}
